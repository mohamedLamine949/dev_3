-- =====================================================================
-- SYNCHRONISATION PROD — à exécuter UNE FOIS dans le SQL Editor Supabase
-- (projet prod kmydbkaytrxtcequngnn). 100% idempotent : ré-exécutable
-- sans risque. Regroupe toutes les migrations nécessaires pour éviter
-- les bugs silencieux (ex. colonne push_token manquante).
-- =====================================================================
...
-- Envoi du push Expo à chaque nouvelle notification
...
BEGIN
  SELECT push_token INTO recipient_push_token FROM public.users WHERE id = NEW.user_id;
  ...
  IF recipient_push_token IS NOT NULL AND recipient_push_token != '' AND recipient_push_token LIKE 'ExponentPushToken[%]' THEN
    ...
    IF has_net_extension THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'Echec push pg_net: %', SQLERRM;
      END;
    END IF;
  END IF;
  RETURN NEW;
...
BEGIN
  ...
  IF conv_record IS NOT NULL THEN
    ...
    SELECT titre INTO annonce_titre FROM public.annonces WHERE id = conv_record.annonce_id;
    INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
    VALUES (recipient_id, sender_name, NEW.contenu, 'chat',
      jsonb_build_object('conversationId', NEW.conversation_id, 'titreAnnonce', COALESCE(annonce_titre, 'Annonce')));
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Notifications annonces (création / paiement / vente)
...
BEGIN
  IF (TG_OP = 'INSERT') THEN
    IF NEW.est_payee = TRUE OR NEW.statut = 'active' THEN
      INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
      VALUES (NEW.user_id, 'Annonce en ligne 🚀', 'Votre annonce "' || NEW.titre || '" est publiée avec succès.', 'annonce_validee', jsonb_build_object('annonceId', NEW.id));
    ELSE
      INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
      VALUES (NEW.user_id, 'Paiement requis 💳', 'Votre annonce "' || NEW.titre || '" a été enregistrée. Payez les frais de mise en ligne pour la publier.', 'paiement_requis', jsonb_build_object('annonceId', NEW.id));
    END IF;
  ELSIF (TG_OP = 'UPDATE') THEN
    IF OLD.est_payee = FALSE AND NEW.est_payee = TRUE THEN
      INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
      VALUES (NEW.user_id, 'Paiement validé ✅', 'Votre paiement a été validé ! Votre annonce "' || NEW.titre || '" est désormais en ligne.', 'annonce_validee', jsonb_build_object('annonceId', NEW.id));
    ELSIF OLD.statut != 'active' AND NEW.statut = 'active' AND NOT (OLD.est_payee = FALSE AND NEW.est_payee = TRUE) THEN
      INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
      VALUES (NEW.user_id, 'Annonce validée 🎉', 'Votre annonce "' || NEW.titre || '" est active et visible par tous.', 'annonce_validee', jsonb_build_object('annonceId', NEW.id));
    ELSIF OLD.statut != 'vendu' AND NEW.statut = 'vendu' THEN
      INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
      VALUES (NEW.user_id, 'Félicitations 🌟', 'Votre annonce "' || NEW.titre || '" est marquée comme vendue. Merci d''avoir choisi Flash Market !', 'annonce_vendue', jsonb_build_object('annonceId', NEW.id));
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER on_annonce_notification
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_annonce_notification();
...
BEGIN
  SELECT titre, user_id INTO annonce_titre, annonce_owner_id FROM public.annonces WHERE id = NEW.annonce_id;
  IF annonce_owner_id IS NOT NULL AND NEW.user_id != annonce_owner_id THEN
    ...
    INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
    VALUES (annonce_owner_id, 'Nouveau favori ⭐', favorited_by_name || ' a ajouté votre annonce "' || COALESCE(annonce_titre, '') || '" à ses favoris.', 'favori', jsonb_build_object('annonceId', NEW.annonce_id));
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE TRIGGER on_favori_notification
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_favori_notification();

-- Notification activation compte pro
...
BEGIN
  IF OLD.type_compte = 'particulier' AND NEW.type_compte = 'professionnel' THEN
    INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
    VALUES (NEW.id, 'Compte Pro Activé 💼', 'Félicitations ! Votre vitrine professionnelle est prête. Personnalisez votre profil dès maintenant.', 'compte_pro_active', jsonb_build_object('userId', NEW.id));
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE TRIGGER on_user_compte_pro_notification
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_user_compte_pro_notification();

-- ---------------------------------------------------------------------
-- 3. NOTIFICATION DE BIENVENUE (à la création du profil)
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.handle_new_user_welcome_notification()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.notifications (user_id, titre, contenu, type, donnees)
  VALUES (NEW.id, 'Bienvenue sur Flash Market 👋', 'Votre compte est prêt ! Publiez votre première annonce et commencez à vendre en toute confiance.', 'bienvenue', '{}'::jsonb);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE TRIGGER on_new_user_welcome_notification
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user_welcome_notification();

-- ---------------------------------------------------------------------
-- 4. SÉCURITÉ ADMIN (admin_users + is_admin + RLS)
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- APRÈS exécution : crée le compte admin (Authentication > Users), puis :
--   INSERT INTO public.admin_users (user_id)
--   SELECT id FROM auth.users WHERE email = 'admin@app-flashmarket.com';
-- =====================================================================
